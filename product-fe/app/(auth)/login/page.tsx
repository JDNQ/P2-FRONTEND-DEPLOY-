...
import { googleLogin, facebookLogin, instagramLogin, twitterLogin } from '@/lib/utils/socialLogin'
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <button type="button" disabled={isLoading} onClick={() => handleSocialLogin('google', googleLogin)}
            className="flex items-center justify-center gap-2 py-2 px-3 border border-outline-variant rounded-xl font-semibold text-xs text-on-surface hover:bg-surface-container-low transition-all active:scale-95 disabled:opacity-50">
            <svg className="w-4 h-4" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05" />
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 3.83-4.53z" fill="#EA4335" />
            </svg>
            Google
          </button>
          ...
          <button type="button" disabled={isLoading} onClick={() => handleSocialLogin('instagram', instagramLogin)}
            className="flex items-center justify-center gap-2 py-2 px-3 border border-outline-variant rounded-xl font-semibold text-xs text-on-surface hover:bg-surface-container-low transition-all active:scale-95 disabled:opacity-50">
            <svg className="w-4 h-4" viewBox="0 0 24 24">
              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069z" fill="url(#ig-gradient)"/>
              <defs><linearGradient id="ig-gradient" x1="0" y1="0" x2="24" y2="24"><stop offset="0" stop-color="#f58529"/><stop offset=".25" stop-color="#dd2a7b"/><stop offset=".5" stop-color="#8134af"/><stop offset=".75" stop-color="#515bd4"/><stop offset="1" stop-color="#00bef0"/></linearGradient></defs>
            </svg>
            Instagram
          </button>
          <button type="button" disabled={isLoading} onClick={() => handleSocialLogin('twitter', twitterLogin)}
            className="flex items-center justify-center gap-2 py-2 px-3 border border-outline-variant rounded-xl font-semibold text-xs text-on-surface hover:bg-surface-container-low transition-all active:scale-95 disabled:opacity-50">

...
export default function ManagerLogsPage() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter(
      (log) =>
        (log.user?.username?.toLowerCase().includes(term) ?? false) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { icon: 'history', bg: 'bg-blue-100', color: 'text-blue-600', label: 'Hành động 24h', value: stats?.totalLogs?.toLocaleString() ?? '—' },
          { icon: 'check_circle', bg: 'bg-green-100', color: 'text-green-600', label: 'Thành công', value: stats ? `${Math.round((stats.actionBreakdown?.length ?? 0) / Math.max(stats.totalLogs, 1) * 100)}%` : '—' },
          { icon: 'warning', bg: 'bg-red-100', color: 'text-red-600', label: 'Cảnh báo bảo mật', value: (stats?.actionBreakdown?.filter(a => a.action.includes('login') || a.action.includes('security')).length ?? 0).toLocaleString() },
          { icon: 'shield', bg: 'bg-purple-100', color: 'text-purple-600', label: 'IP ghi nhận', value: logs.filter(l => l.ip).length.toLocaleString() },
        ].map((s) => (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#c4c5d9]/50 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#f5f2ff] border-b border-[#c4c5d9]">
                {['Thời gian', 'Người thực hiện', 'Hành động', 'Đối tượng', 'IP', 'Chi tiết'].map((h) => (
                ...
                filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-[#f5f2ff] transition-all cursor-default">
                    <td className="px-6 py-5 align-top">
                      <p className="text-sm text-[#1e40af]">{new Date(log.createdAt).toLocaleTimeString('vi-VN')}</p>
                      <p className="text-[12px] text-[#747688]">{new Date(log.createdAt).toLocaleDateString('vi-VN')}</p>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold">{log.user?.username ?? 'Hệ thống'}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      ...
                      {log.details && <p className="text-[12px] text-[#747688] mt-1 italic">{log.details}</p>}
                    </td>
                    <td className="px-6 py-5">
                      {log.entity ? (
                        <div className="px-3 py-1 rounded-lg inline-flex items-center gap-2 bg-[#e8e6ff]">
                          <span className="material-symbols-outlined text-sm text-[#1e40af]">settings</span>
                          <span className="text-sm text-[#1e40af]">{log.entity}#{log.entityId}</span>
                        </div>
                      ) : (
                        <span className="text-sm text-[#747688]">—</span>
                      )}
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-right">
                      <span className="px-3 py-1 text-sm rounded-full border bg-green-100 text-green-700 border-green-200">
                        {log.details ? 'Có chi tiết' : '—'}
                      </span>
                    </td>
                  </tr>
                ))
        ...
        <div className="px-6 py-4 bg-[#f5f2ff] flex items-center justify-between border-t border-[#c4c5d9]">
          <p className="text-[12px] text-[#747688]">Hiển thị {filteredLogs.length} trong tổng số {stats?.totalLogs ?? 0} hành động</p>
          <div className="flex items-center gap-2">
